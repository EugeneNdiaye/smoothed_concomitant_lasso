...
# @cython.wraparound(False)
# @cython.cdivision(True)
# cdef double bound_screen(double abs_XTy_j, double nrm_y, double sqrt_n,
#                          double eta_sup, double eta_inf) nogil:

#     cdef double gamma_sup = eta_sup * sqrt_n / nrm_y
#     cdef double gamma_inf = eta_inf * sqrt_n / nrm_y
#     cdef double bound = 0.
#     cdef double double_tmp = 0.

#     if fabs(abs_XTy_j) > gamma_sup:

#         double_tmp = sqrt(1 - gamma_sup ** 2) * \
#                      sqrt(1 - fabs(abs_XTy_j) ** 2)
#         bound = gamma_sup * fabs(abs_XTy_j) + double_tmp

#     elif fabs(abs_XTy_j) < gamma_inf:

#         double_tmp = sqrt(1 - gamma_inf ** 2) * \
#                      sqrt(1 - fabs(abs_XTy_j) ** 2)
#         bound = gamma_inf * fabs(abs_XTy_j) + double_tmp
...
def cd_smoothed_concomitant_fast(double[::1, :] X, double[:] y, double[:] beta,
    ...
    cdef double gamma_sup = 0.
    cdef double gamma_inf = 0.
    # cdef double eta_sup = 0.
    # cdef double eta_inf = 0.
    cdef double p_obj = 0.
    cdef double d_obj = 0.
    cdef double bound = 0.
    cdef double sqrtn_over_nrmy = sqrt_n / nrm_y
    cdef int yes_screen = False

    cdef double[:] l_sqn_over_norm_X = np.zeros(n_features, order='F')
